...
function getSimpleActInfoVo() {
    let config = {
        url: `https://lzdz-isv.isvjcloud.com/customer/getMyPing?userId=1000361242&token=${$.token2}&fromType=APP`,
        headers: {
            ...
            'content-type': 'application/x-www-form-urlencoded',
            'cookie': `IsvToken=${$.isvToken};LZ_TOKEN_KEY=lztokenpage31a5f8c338564ef0b987b8df742b696e;LZ_TOKEN_VALUE=xv52nUHhZeNts/pGPfsHoQ;AUTH_C_USER=;`,
        }
    }
    // console.log(config)
...
function getinfo() {
    let config = taskUrl("/dingzhi/qqxing/pasture/myInfo", `activityId=90121061401&pin=${$.pin}&actorUuid=&userUuid=`)
    return new Promise(resolve => {
        $.get(config, async (err, resp, data) => {
            try {
                if (err) {
                  ...
                  if (data.result) {
                      ...
                      console.log(`当前🐮🐮成长值：${$.score}  饲料：${$.food.totalNum-$.food.useNum}  抽奖次数：${$.draw.totalNum-$.draw.useNum}  签到天数：${$.sign.totalNum}`)
